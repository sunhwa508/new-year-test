...
import { semesterQuestions } from "@/data/semester-questions";
import { ScoreMap } from "@/data/semester-results";
...
export default function SemesterQuizPage() {
  const router = useRouter();
  const [currentQuestion, setCurrentQuestion] = useState(0);
  const [scores, setScores] = useState<ScoreMap>({});
  const [history, setHistory] = useState<AnswerHistory[]>([]);

  const question = semesterQuestions[currentQuestion];
  const progress = ((currentQuestion + 1) / semesterQuestions.length) * 100;
  const comment = getProgressComment(currentQuestion, semesterQuestions.length);

  const handleAnswer = (answerScores: { [type: string]: number }) => {
    // 히스토리에 현재 답변 저장
    setHistory([
      ...history,
      { questionIndex: currentQuestion, scores: answerScores },
    ]);
    ...
    if (currentQuestion < semesterQuestions.length - 1) {
      setCurrentQuestion(currentQuestion + 1);
    } else {
      localStorage.setItem("semesterQuizScores", JSON.stringify(newScores));
      router.push("/semester/result");
    }
  };
          ...
          <div className="flex justify-between items-center text-sm mb-2">
            <div className="flex items-center gap-3">
              ...
              <span className="text-stone-400">
                {currentQuestion + 1} / {semesterQuestions.length}
              </span>
            </div>
        ...
        <div className="space-y-3">
          {question.answers.map((answer) => (
            <button
              ...
              className="w-full p-4 text-left bg-white border border-stone-200 hover:border-purple-400 rounded-xl transition-colors duration-200 text-stone-700 hover:text-stone-900 font-medium"
            >
              {answer.text}
            </button>
          ))}
